...
void main( )
{
    ...
    uv2.y = -uv2.y;
    vec3 col = texture2D(image, uv2).xyz;;
    vec4 uv = vec4( uv2, uv2 - (rcpFrame * (0.5 + FXAA_SUBPIX_SHIFT)));
    vec3 fxaa = FxaaPixelShader( uv, image, 1./resolution.xy );
    float divide = sin(time * 0.001) * 0.5 + 0.5;
    gl_FragColor = (uv2.x > divide - 0.01 && uv2.x < divide + 0.01) ? vec4(vec3(0.),1.) : vec4(uv2.x > divide ? fxaa : col,1.);
}